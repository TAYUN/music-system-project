<template>
	<div class="table">
		...
		<el-table
			...
			<el-table-column prop="gqName" label="歌手-歌名" align="center"></el-table-column>
		...
		<el-dialog title="添加歌曲" :visible.sync="centerDialogVisible" width="400px" center>
			<el-form label-width="80px" action="" id="tf">
				<el-form-item prop="" label="歌手名字">
					<el-select
						v-model="value"
						filterable
						placeholder="请选择"
						@focus="showSingerName">
						<el-option
				...
				<el-form-item prop="" label="歌曲名字">
					<el-select
						v-model="song"
						filterable
						placeholder="请选择"
						@focus="showSongNameBySingerName">
						<el-option
	...
	methods: {
		...
		showSongNameBySingerName() {
			...
			songOfSingerId(this.value).then((res) => {
				...
				for (var i = 0; i < res.length; i++) {
					console.log('i : ', res[i]);
					let obj = {};
					obj.songValue = res[i].id;
					obj.songLabel = res[i].gqName;
					arr.push(obj);
				}
				this.songOptions = arr;
		...
		showSingerName() {
			getAllSinger().then((res) => {
				this.options = res.filter(function (el) {
					return (el.value = el.id), (el.label = el.name);
				});
			});
		},
		//根据歌单id查询歌曲列表
